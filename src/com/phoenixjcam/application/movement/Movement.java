package com.phoenixjcam.application.movement;
...
public class Movement {
	...
	if (map.getTile(topTile, leftTile) == 0
		|| map.getTile(topTile, leftTile) == 2) {
	    topLeft = true;
	} else {
	    topLeft = false;
	}
	...
	if (map.getTile(topTile, rightTile) == 0
		|| map.getTile(topTile, rightTile) == 2) {
	    topRight = true;
	} else {
	    topRight = false;
	}
	...
	if (map.getTile(bottomTile, leftTile) == 0
		|| map.getTile(bottomTile, leftTile) == 2) {
	    bottomLeft = true;
	} else {
	    bottomLeft = false;
	}
	...
	if (map.getTile(bottomTile, rightTile) == 0
		|| map.getTile(bottomTile, rightTile) == 2) {
	    bottomRight = true;
	} else {
	    bottomRight = false;
	}
	...
	if (nextPosition.y < 0) {
	    if (topLeft || topRight) {
		nextPosition.y = 0;
		newPosition.y = rowTile * map.getTileSize()
			+ player.getHeight() / 2;
	    } else {
	...
	if (nextPosition.y > 0) {
		...
		newPosition.y = (rowTile + 1) * map.getTileSize()
			- player.getHeight() / 2;
	    } else {
		newPosition.y += nextPosition.y;
	    }
	...
	if (nextPosition.x < 0) {
		...
		nextPosition.x = 0;
		newPosition.x = coulmnTile * map.getTileSize()
			+ player.getWidth() / 2;
	    } else {
	...
	if (nextPosition.x > 0) {
		...
		nextPosition.x = 0;
		newPosition.x = (coulmnTile + 1) * map.getTileSize()
			- player.getWidth() / 2;
	    } else {
		newPosition.x += nextPosition.x;
	    }
	...
	g.drawImage(player.getSprite(), x, y, null);
    }
}
